...
#define LCDHardWareSPI     (0) //宏定义是否使用硬件SPI，使用则宏定义为1，不使用则宏定义为0
#if(LCDHardWareSPI)
#include "msp430f5529_spi.h"    //需要包含spi.h的文件
//如果使用硬件SPI则需宏定义以下函数
//硬件SPI初始化函数，时钟线在没有数据传输时为高电平，主机先下降沿接收数据，再在上升沿发送数据
//波特率太慢也会显示乱码，不知道为什么
#define LCDHardWareSPI_Init()            SPI_Master_Init(SPI2,50MHz,SPI_MSB_First,SPI_STE_None,SPI_CLK_Mode_4)
...
#define LCD_SDI_PORT   P2
#define LCD_SDI_PIN    2
