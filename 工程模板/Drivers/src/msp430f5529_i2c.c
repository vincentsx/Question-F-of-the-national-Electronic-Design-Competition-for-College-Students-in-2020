...
/*******************************************************************************
*  函数名称： MemoryRead(I2Cn i2cn)
*  功能说明： 读取MLX90614中的温度数据
*  参数说明： I2Cn i2cn      :模块号
*  函数返回： DataH*256 + DataL
*  使用示例： 无
********************************************************************************/
uint32_t MemoryRead(I2Cn i2cn)    
{
  uint8_t DataL,DataH,Pecreg;
  I2C_EnterSend (i2cn);
  I2C_Start     (i2cn);                        //发送一个起始信号
  I2C_SendByte  (i2cn,0x07);                   //发送要写入的命令
  
  I2C_WaitBusy  (i2cn);                        //等待传输完毕
  I2C_Start     (i2cn);                        //发送一个起始信号
  I2C_SendByte  (i2cn,0x01); 
  ...
  DataL = I2C_ReadByte(i2cn);
  I2C_WaitBusy  (i2cn);                        //等待传输完毕

  DataH = I2C_ReadByte(i2cn);
  I2C_WaitBusy  (i2cn);                        //等待传输完毕
  I2C_Stop      (i2cn);                        //发送一个停止信号,读的话要先发送停止位

  Pecreg = I2C_ReadByte(i2cn);
  I2C_WaitBusy  (i2cn);                        //等待传输完毕
